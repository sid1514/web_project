...
const CollectionCar=()=>{
 ...
 const handleAddData=()=>{
  ...
  const handleRemove=(car_model)=>{
    axios.delete(`http://localhost:4000/removeCar/${car_model}`)
    .then((response) => {
      if (response.data === "not found ....") {
        ...
        setCarData(prevCarData => prevCarData.filter(c => c.car_model !== car_model));
      }
    })
    .catch((error) => {
      console.error(error);
  ...
  return (
    ...
    <div className='carsTable_container'>
     <br></br>
      ...
      <table className='cars_table'>
       <th><h4>S.r. no.</h4></th>
          <th><h4>car brand</h4> </th> 
          <th><h4>car model</h4> </th> 
          <th><h4>car price</h4> </th> 
          <th><h4>car image</h4></th> <th><h4>number_of_seats</h4></th> <th><h4>Drive Type</h4></th>
      <th><h4>transmission</h4></th> <th><h4>year</h4></th> <th><h4>fuel type</h4></th> <th><h4>Engine size</h4></th> <th><h4>doors</h4> </th> <th><h4>cylinder</h4></th> <th><h4>color</h4></th>
    ...
    <td><h5>{c.car_model}</h5></td>   
    <td><h5>{c.car_price} </h5> </td>
    <td><h5>{c.car_image}</h5></td>  
     <td><h5>{c.number_of_seats}</h5></td>
     <td><h5>{c.Drive_Type}</h5></td>
     <td><h5>{c.transmission}</h5></td>
     <td><h5>{c.year}</h5></td>
     <td><h5>{c.fuel_type}</h5></td>
     <td><h5>{c.Engine_size}</h5></td>
     <td><h5>{c.doors}</h5></td>
     <td><h5>{c.cylinder}</h5></td>
     <td><h5>{c.color}</h5></td>
     <td><button onClick={() => handleRemove(c.car_model)}><Icon name='remove'/> </button> </td>
     </tr>):null
...
<div className='Input_container'>
  ...
  <tr>
  <Input placeholder="enter brand name" label="car brand" onChange={(e)=>{setCarBrand(e.target.value)}}></Input>
  </tr>
  <br></br>
<tr>
  <Input placeholder="enter car model" label="car model" onChange={(e)=>{setCarModel(e.target.value)}}></Input>
</tr>
<br></br>
  <tr>
  <Input placeholder="enter car price" type="number" label="car price" onChange={(e)=>{setCarPrice(e.target.value)}}></Input>
  </tr>
  <br></br>
  <tr>
  <Input type="file" placeholder="upload image" label="car image" onChange={(e)=>{setCarImage(e.target.files[0])}}></Input>
  <button style={{width:'100px',height:'50px'}} >upload</button>
  </tr>
  <br></br>
  <tr>
  <Input placeholder="enter Engine size" label="Engine Size" onChange={(e)=>{setEngineSize(e.target.value)}}></Input>
  </tr>
  <br></br>
  <tr>
  <Input placeholder="number of doors" label="doors" onChange={(e)=>{setDoors(e.target.value)}}></Input>
  </tr>
  <br></br>
  <tr>
  <Input placeholder="cylinder contain" label="cylinder" onChange={(e)=>{setCylinder(e.target.value)}}></Input>
  </tr>
  <br></br>
  <tr>
  <Input placeholder="color" label="car color" onChange={(e)=>{setColor(e.target.value)}}></Input>
  </tr>
</div>
...
<br></br>
  <Button inverted color="olive" className='add_button' onClick={handleAddData}>ADD</Button>
